...
async function cargarTabla() {
    ...
    try {
        ...
        const res = await GetString(ctrl.coberturas + 'listar');
        if (res == "") {
            $("#cuerpoTabla").html("<tr><td colspan='3' class='text-center'>No existen registros</td></tr>");
        } else {
...
function redirect(id) {
    const ctrl = new Controladores();
    location.href = ctrl.coberturas + "Cobertura/?id=" + id;
}
...
async function eliminar(id) {
    ...
    try {
        const ctrl = new Controladores();
        const res = await PostIdResString(ctrl.coberturas + "eliminar", id);
        if (res == "ok") {
